...
LD_LIBRARY_PATH=$DPISIM_HOME/dpisim_libs  $DPISIM_HOME/dpisim --routing --flavour=$PLAT --backend-config-file=$CONFIG --service-set=$SS --filter-down="tcp and src net 10.0.2.0/24" --filter-up="tcp and src net 10.0.0.0/24 or src net 10.0.1.0/24" -c=$CELL_IP -k=255.255.255.0 --devDown=$DEV_DOWN --devUp=$DEV_UP,$DEV_UP2 --log-file=$LOGFILE --imsi=56000000056 --msisdn=1234567890 --sgsn=1.2.2.1 --ggsn=1.2.3.4 --charging-id=978  $LOGSPARS --routing-timer=true 
